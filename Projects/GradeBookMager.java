...
public class GradeBookMager{
    ...
    private static void calculateAverageGradesForEachStudent(Scanner scanner){
        for(int i=0;i<numStudents;i++){
    ...
    private static void calculateAverageGradesForEachSubject(Scanner scanner){
    ...
    private static void searchForStudent(Scanner scanner){
        System.out.println("Enter student  identifier to search ");
            System.out.print("enter the identifier : ");
            int id = scanner.nextInt();
            for(int i=0;i<numStudents;i++){
            if(students[i].getIdentifier()==id){
                System.out.println("Student with identifier "+students[i].getIdentifier()+"found\n"+"Name:"+students[i].getName()+"\nGreads\n"+students[i].getGrades()[i]);
            }
    }
        
    }
    private static void sortStudentsByAverageGrade(Scanner scanner){
        System.out.println("Students sorted by average grade(descending order)");
       
        for(int i=0;i<numStudents;i++){
            Student student=students[i];
             int max[] = new int[50];
             max[i]=students[i].getGrades()[0];
            for(int j=1;j<=student.getNumOfGrades();j++){
                if(max[i]<student.getGrades()[j]){
                    max[i] = max[i]+student.getGrades()[j];
                    student.getGrades()[j]= max[i] -student.getGrades()[j];
                    max[i] = max[i] - student.getGrades()[j];
                }else{
                    max[i]=student.getGrades()[j];
                }
            }
                System.out.println(student.getName()+"(Average Grade:"+max[i]+")");
        }
    }
